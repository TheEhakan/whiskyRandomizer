...
router.get('/bottles', authorization, async (req, res) => {
    ...
    try {
        ...
        const bottles = await pool.query('SELECT * FROM bottles WHERE user_id = $1', [req.user]);
...
router.post('/bottles', authorization, async (req, res) => {
    ...
    //get info from user
    const { bottle_name, bottle_type, bottle_neat, bottle_iced, bottle_mixed } = req.body;
    ...
    try {
        ...
        const bottle = await pool.query('INSERT INTO bottles (bottle_name, bottle_type, bottle_neat, bottle_iced, bottle_mixed, user_id) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *;', [bottle_name, bottle_type, bottle_neat, bottle_iced, bottle_mixed, req.user]);

        res.json(bottle.rows);
    } catch (error) {
        console.error(error.message);
        res.status(500).json('Server Error');
    }
...
router.put('/bottles', authorization, async (req, res) => {
    ...
    const { bottle_id, bottle_name, bottle_type, bottle_neat, bottle_iced, bottle_mixed } = req.body;
    ...
    try {
        ...
        const bottle = await pool.query('UPDATE bottles SET bottle_name = $2, bottle_type = $3, bottle_neat = $4, bottle_iced = $5, bottle_mixed = $6 WHERE bottle_id = $1 AND user_id = $7 RETURNING *;', [bottle_id, bottle_name, bottle_type, bottle_neat, bottle_iced, bottle_mixed, req.user]);
...
router.delete('/bottles', authorization, async (req, res) => {
    ...
    try {
        ...
        res.json(bottle ? 'Bottle Deleted' : 'Error');
    } catch (error) {
        console.error(error.message);
        res.status(500).json('Server Error');
    }
...
router.get('/cocktails', authorization, async (req, res) => {
    ...
    try {
        ...
        const cocktails = await pool.query('SELECT * FROM cocktails WHERE user_id = $1', [req.user]);
...
router.get('/settings', authorization, async (req, res) => {
    ...
    try {
        ...
        res.json(settings.rows[0].user_settings);
